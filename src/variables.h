...
// Methods
	...
	std::unique_ptr<GeneralizedOffLatticeModel3D<T,SurfaceData> > createModel(TriangleFlowShape3D<T,SurfaceData>* flowShape,
		const int& flowType);

	std::unique_ptr<OffLatticeBoundaryCondition3D<T,Descriptor,BoundaryType> > createBC(GeneralizedOffLatticeModel3D<T,SurfaceData>* model,
