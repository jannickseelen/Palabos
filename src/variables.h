...
// Methods
	...
	std::unique_ptr<OffLatticeModel3D<T,Descriptor> > createModel(TriangleFlowShape3D<T,SurfaceData>* flowShape,
		const int& flowType);

	std::unique_ptr<OffLatticeBoundaryCondition3D<T,Descriptor,BoundaryType> > createBC(OffLatticeModel3D<T,Descriptor>* model,
