...
void saveState(std::vector<MultiBlock2D*> blocks, plint iteration, bool saveDynamicContent, FileName xmlFileName,
FileName baseFileName, plint fileNamePadding){
    std::string fname_base;
	if(sizeof(fileNamePadding)<sizeof(int)){
		fname_base = createFileName(baseFileName.get(), iteration, fileNamePadding);}
	else{ throw std::overflow_error("saveState");}
