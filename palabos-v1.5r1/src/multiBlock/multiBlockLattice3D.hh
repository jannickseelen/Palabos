...
template<typename T, template<typename U> class Descriptor>
MultiBlockLattice3D<T,Descriptor>& MultiBlockLattice3D<T,Descriptor>::operator+=(MultiBlockLattice3D<T,Descriptor>& rhs)
{
	std::map<plint,BlockLattice3D<T,Descriptor>*> overload;
	std::map<plint,BlockLattice3D<T,Descriptor>*> otherBlockMap = rhs.getBlockLattices();
	typename std::map<plint,BlockLattice3D<T,Descriptor>*>::iterator it;
	for(it = otherBlockMap.begin(); it!=otherBlockMap.end(); ++it){
		std::pair<plint,BlockLattice3D<T,Descriptor>*> pair(it->first,it->second);
		overload.insert(pair);
	}
	for(it = blockLattices.begin(); it!=blockLattices.end(); ++it){
		std::pair<plint,BlockLattice3D<T,Descriptor>*> pair(it->first,it->second);
		overload.insert(pair);
	}
	this->blockLattices = overload;
	return *this;
}
