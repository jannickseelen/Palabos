...
const char* MultiBlockSerializer3D::getNextDataBuffer(pluint& bufferSize) const {
        ...
        ordering==IndexOrdering::memorySaving)
    {
        ...
        if (iZ+nextChunkSize>domain.z1+1) {
            nextChunkSize = (int)domain.z1-(int)iZ+1;
        }
        if (hasData) {
            computeBufferAlongZ(nextBlockId, nextChunkSize);
            bufferSize = nextChunkSize*(int)multiBlock.sizeOfCell();
        }
        else {
            if (ordering==IndexOrdering::forward) {
        ...
        if (iX+nextChunkSize>domain.x1+1) {
            nextChunkSize = (int)domain.x1-(int)iX+1;
        }
        if (hasData) {
...
bool MultiBlockSerializer3D::isLocal(plint blockId) const {
    return multiBlock.getMultiBlockManagement().
               getThreadAttribution().isLocal((int)blockId);
}

void MultiBlockSerializer3D::computeBufferAlongX ( plint nextBlockId,
                                                   int nextChunkSize ) const
{
    int bufferSize = nextChunkSize*(int)multiBlock.sizeOfCell();
...
void MultiBlockSerializer3D::computeBufferAlongY (
        plint nextBlockId, int nextChunkSize ) const
{
    int bufferSize = nextChunkSize*(int)multiBlock.sizeOfCell();
    bool blockIsLocal = isLocal(nextBlockId);
    if (blockIsLocal) {
...
void MultiBlockSerializer3D::communicateBuffer (
        int bufferSize, plint fromBlockId, bool isAllocated ) const
{
    ...
    if (!isAllocated && global::mpi().isMainProcessor()) {
        int fromProc = multiBlock.getMultiBlockManagement().
                           getThreadAttribution().getMpiProcess((int)fromBlockId);
        // Avoid pointing to a buffer of size 0, as this leads to undefined behavior.
        PLB_ASSERT(bufferSize>0);
        buffer.resize(bufferSize);
        MPI_Request request;
...
void MultiBlockSerializer3D::fillBufferWithZeros(int nextChunkSize) const {
    int bufferSize = nextChunkSize*(int)multiBlock.sizeOfCell();
    if (global::mpi().isMainProcessor()) {
        // Avoid pointing to a buffer of size 0, as this leads to undefined behavior.
        PLB_ASSERT(bufferSize>0);
...
void MultiBlockUnSerializer3D::fillBufferAlongX(plint nextBlockId, int nextChunkSize) {
    int bufferSize = nextChunkSize*(int)multiBlock.sizeOfCell();
    bool blockIsLocal = isLocal(nextBlockId);
    communicateBuffer(bufferSize, nextBlockId, blockIsLocal);
    if (blockIsLocal) {
...
void MultiBlockUnSerializer3D::fillBufferAlongY(plint nextBlockId, int nextChunkSize) {
    int bufferSize = nextChunkSize*(int)multiBlock.sizeOfCell();
    bool blockIsLocal = isLocal(nextBlockId);
    communicateBuffer(bufferSize, nextBlockId, blockIsLocal);
    if (blockIsLocal) {
...
void MultiBlockUnSerializer3D::fillBufferAlongZ(plint nextBlockId, int nextChunkSize) {
    int bufferSize = nextChunkSize*(int)multiBlock.sizeOfCell();
    bool blockIsLocal = isLocal(nextBlockId);
    communicateBuffer(bufferSize, nextBlockId, blockIsLocal);
    if (blockIsLocal) {
...
void MultiBlockUnSerializer3D::communicateBuffer (
        int bufferSize, plint toBlockId, bool isAllocated ) const
{
#ifdef PLB_MPI_PARALLEL
    if (isAllocated && !global::mpi().isMainProcessor()) {
        // Avoid pointing to a buffer of size 0, as this leads to undefined behavior.
    ...
    if (!isAllocated && global::mpi().isMainProcessor()) {
        int toProc = multiBlock.getMultiBlockManagement().
                         getThreadAttribution().getMpiProcess((int)toBlockId);
