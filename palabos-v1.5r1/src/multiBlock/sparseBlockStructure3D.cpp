...
std::vector<plint> SparseBlockStructure3D::getLocalBlocks(ThreadAttribution const& attribution) const
{
    ...
    for (; it != bulks.end(); ++it) {
        ...
        if (attribution.isLocal((int)blockId)) {
...
bool EuclideanIterator3D::getNextChunkX( plint iX, plint iY, plint iZ,
                                         plint& blockId, int& chunkSize ) const
{
    ...
    if (blockId == -1) {
        ...
        chunkSize = (int)exploreX-(int)iX;
        return false;
    }
    else {
        Box3D bulk;
        sparseBlock.getBulk(blockId, bulk);
        chunkSize = (int)bulk.x1-(int)iX+1;
        return true;
    }
}
                                         ...
                                         plint& blockId, int& chunkSize ) const
{
    ...
    if (blockId == -1) {
        ...
        chunkSize = (int)exploreY-(int)iY;
        return false;
    }
    else {
        Box3D bulk;
        sparseBlock.getBulk(blockId, bulk);
        chunkSize = (int)bulk.y1-(int)iY+1;
        return true;
    }
}
                                         ...
                                         plint& blockId, int& chunkSize ) const
{
    ...
    if (blockId == -1) {
        ...
        chunkSize = (int)exploreZ-(int)iZ;
        return false;
    }
    else {
        Box3D bulk;
        sparseBlock.getBulk(blockId, bulk);
        chunkSize = (int)bulk.z1-(int)iZ+1;
        return true;
    }
}
