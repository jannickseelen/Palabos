...
bool SerialThreadAttribution::isLocal(int blockId) const {
    return true;
}
...
int SerialThreadAttribution::getMpiProcess(int blockId) const {
    return global::mpi().bossId();
}

int SerialThreadAttribution::getLocalThreadId(int blockId) const {
    return 0;
}
...
bool OneToOneThreadAttribution::isLocal(int blockId) const {
...
int OneToOneThreadAttribution::getMpiProcess(int blockId) const {
    return blockId;
}
int OneToOneThreadAttribution::getLocalThreadId(int blockId) const {
    return 0;
}
            ...
            std::map<plint,std::vector<plint> > const& remappedIds ) const
{
    ...
    for (; it != remappedIds.end(); ++it) {
        ...
        for (int iNew=0; iNew<newIds.size(); ++iNew) {
            newThreadAttribution->addBlock (
                    (int)newIds[iNew],
                    rhs.getMpiProcess((int)oldId),
                    rhs.getLocalThreadId((int)oldId) );
        }
    }
              ...
              std::vector<plint> const& mpiProcesses, std::vector<plint> const& localThreads ) const
{
    ...
    ExplicitThreadAttribution* newThreadAttribution
        = new ExplicitThreadAttribution();
    for (int iNew=0; iNew<ids.size(); ++iNew) {
        newThreadAttribution->addBlock (
                (int)ids[iNew], mpiProcesses[iNew], localThreads[iNew] );
    }
    return newThreadAttribution;
}
...
void ExplicitThreadAttribution::addBlock (
        int blockId, plint mpiProcess, plint localThread )
{
    mpiProcessAttribution[blockId] = mpiProcess;
    localThreadAttribution[blockId] = localThread;
}

bool ExplicitThreadAttribution::isLocal(int blockId) const {
...
int ExplicitThreadAttribution::getMpiProcess(int blockId) const {
    std::map<plint,plint>::const_iterator it = mpiProcessAttribution.find(blockId);
    // If this blockId is not registered, fall back to default behavior,
    //   which is the one specified by OneToOneThreadAttribution.
    if (it == mpiProcessAttribution.end()) {
...
int ExplicitThreadAttribution::getLocalThreadId(int blockId) const {
