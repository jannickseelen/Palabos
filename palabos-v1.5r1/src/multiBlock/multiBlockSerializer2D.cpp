...
const char* MultiBlockSerializer2D::getNextDataBuffer(pluint& bufferSize) const {
        ...
        ordering==IndexOrdering::memorySaving)
    {
        ...
        if (iY+nextChunkSize>domain.y1+1) {
            nextChunkSize = (int)domain.y1-(int)iY+1;
        }
        if (hasData) {
...
void MultiBlockSerializer2D::fillBufferWithZeros(int nextChunkSize) const {
    int bufferSize = nextChunkSize*(int)multiBlock.sizeOfCell();
    if (global::mpi().isMainProcessor()) {
        // Avoid pointing to a buffer of size 0, as this leads to undefined behavior.
        PLB_ASSERT(bufferSize>0);
...
void MultiBlockUnSerializer2D::fillBufferAlongX(plint nextBlockId, int nextChunkSize) {
    int bufferSize = nextChunkSize*(int)multiBlock.sizeOfCell();
    bool blockIsLocal = isLocal(nextBlockId);
    communicateBuffer(bufferSize, nextBlockId, blockIsLocal);
    if (blockIsLocal) {
...
void MultiBlockUnSerializer2D::fillBufferAlongY(plint nextBlockId, int nextChunkSize) {
    int bufferSize = nextChunkSize*(int)multiBlock.sizeOfCell();
    bool blockIsLocal = isLocal(nextBlockId);
    communicateBuffer(bufferSize, nextBlockId, blockIsLocal);
    if (blockIsLocal) {
