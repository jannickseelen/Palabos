...
RandomRedistribute3D::RandomRedistribute3D(int rseed_)
    : rseed(rseed_)
{ }
        ...
        MultiBlockManagement3D const& original ) const
{
    ...
    std::vector<std::pair<int,int> > blockToProc(domains.size());
    std::map<plint,Box3D>::const_iterator it = domains.begin();
    for (int pos=0; it != domains.end(); ++it, ++pos) {
        int blockId = (int)it->first;
        int procId = (int)originalAttribution.getMpiProcess(blockId);
        blockToProc[pos] = std::pair<int,int>(blockId,procId);
    }
