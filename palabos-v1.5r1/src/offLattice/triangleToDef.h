...
template<typename T>
void constructSurfaceMesh(std::vector<typename TriangleSet<T>::Triangle> const& _triangles, std::vector<Array<T,3> >& _vertexList,
	std::vector<plint>& _emanatingEdgeList, std::vector<Edge>& _edgeList, T _epsilon=std::numeric_limits<float>::epsilon()){
	TriangleToDef<T> def(_triangles, _epsilon);
	def.generateOnce(_vertexList, _emanatingEdgeList, _edgeList);
}
