...
public:
    VoxelizeMeshFunctional3D (TriangularSurfaceMesh<T> const& mesh_ );
...
private:
    bool checkIfFacetsCrossed (AtomicContainerBlock3D& hashContainer, Array<T,3> const& point1, Array<T,3> const& point2,
							T& distance, plint& whichTriangle);
    bool distanceToSurface (AtomicContainerBlock3D& hashContainer, Array<T,3> const& point, T& distance, bool& isBehind) const;
    bool createVoxelizationRange (Box3D const& domain, ScalarField3D<int>& voxels, Array<plint,2>& xRange, Array<plint,2>& yRange,
							Array<plint,2>& zRange );
    bool voxelizeFromNeighbor(ScalarField3D<int> const& voxels, AtomicContainerBlock3D& hashContainer,
							const Dot3D& pos, const Dot3D& neighbor, int& voxelType, const int& verificationLevel);
    void printOffender (ScalarField3D<int> const& voxels, AtomicContainerBlock3D& hashContainer, const Dot3D& pos);
private:
    TriangularSurfaceMesh<T> const& mesh;
};
