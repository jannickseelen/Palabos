                                                     ...
                                                     ThreadAttribution const& oldThreadAttribution )
{
    ...
    for (; it != domains.end(); ++it) {
        plint id = it->first;
        if (oldThreadAttribution.isLocal((int)id)) {
    ...
    for (plint iThread=0; iThread<global::mpi().getSize(); ++iThread) {
        ...
        for (plint iBlock=startBlock; iBlock<endBlock; ++iBlock) {
            newThreadAttribution->addBlock((int)everyOnesBlocks[iBlock],iThread);
        }
    }
...
void repartitionIter(SparseBlockStructure3D const& sparseBlock, std::set<plint>& localBlocks, plint myPartner, bool exchangeAnyway)
{
    double epsilon = 0.1;
    if (exchangeAnyway) {
        epsilon =1.0;
    ...
    plint receivedBlock;
    global::mpi().sendRecv(&sacrificedBlock, &receivedBlock, 1, (int)myPartner, (int)myPartner);
    plint partnerTotalContactsBefore;
    global::mpi().sendRecv(&totalContactsBefore, &partnerTotalContactsBefore, 1, (int)myPartner, (int)myPartner);
    ...
    global::mpi().sendRecv(&totalContactsAfter, &partnerTotalContactsAfter, 1, (int)myPartner, (int)myPartner);
