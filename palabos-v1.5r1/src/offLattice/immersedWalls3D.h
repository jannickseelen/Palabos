        ...
        bool incompressibleModel)
{
    ...
    std::unique_ptr<MultiScalarField3D<T> > rhoBar = generateMultiScalarField<T>(lattice, envelopeWidth);
    computeRhoBar<T,Descriptor>(lattice, *rhoBar, domain);
    std::unique_ptr<MultiScalarField3D<T> > copiedRhoBar = generateMultiScalarField<T>(*rhoBar, envelopeWidth);
    plb::copy(*rhoBar, *copiedRhoBar, rhoBar->getBoundingBox());

    std::unique_ptr<MultiTensorField3D<T,SymmetricTensorImpl<T,3>::n> > PiNeq =
        generateMultiTensorField<T,SymmetricTensorImpl<T,3>::n>(lattice, envelopeWidth);
    computePiNeq<T,Descriptor>(lattice, *PiNeq, domain);
    std::unique_ptr<MultiTensorField3D<T,SymmetricTensorImpl<T,3>::n> > copiedPiNeq =
