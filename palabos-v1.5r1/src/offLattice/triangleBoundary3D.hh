        ...
        plint referenceDirection_, Dot3D location )
{
	...
	std::vector<typename TriangleSet<T>::Triangle> triangles = triangleSet_.getTriangles();
    constructSurfaceMesh<T>(triangles, vertexList, emanatingEdgeList, edgeList, eps);
      ...
      boundary(boundary_)
{
    ...
    std::unique_ptr<MultiScalarField3D<int> > fullVoxelMatrix = voxelize( boundary.getMesh(), boundary.getMargin()+extraLayer_,
		borderWidth );
      ...
      boundary(boundary_)
{
    ...
    std::unique_ptr<MultiScalarField3D<int> > fullVoxelMatrix (voxelize( boundary.getMesh(), boundingBox, borderWidth ) );
    fullVoxelMatrix->setRefinementLevel(gridLevel_);
    createSparseVoxelMatrix(*fullVoxelMatrix, blockSize_, envelopeWidth_);
    createTriangleHash();
    boundary.popSelect();
