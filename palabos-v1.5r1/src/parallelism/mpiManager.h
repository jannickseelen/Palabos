...
public:
    ...
    void scatterV(T *sendBuf, int* sendCounts, int* displs, T* recvBuf, int recvCount, int root){
		scatterv_impl(sendBuf, sendCounts, displs, recvBuf, recvCount, root);}
	...
	template <typename T>
    void gatherV(T* sendBuf, int sendCount, T* recvBuf, int* recvCounts, int* displs, int root){
		gatherv_impl(sendBuf, sendCount, recvBuf, recvCounts, displs, root);}
...
private:
    ...
    template <typename T>
    void scatterv_impl(T *sendBuf, int* sendCounts, int* displs, T* recvBuf, int recvCount, int root);

    /// Implementation code for Gather
	template <typename T>
    void gatherv_impl(T* sendBuf, int sendCount, T* recvBuf, int* recvCounts, int* displs, int root);
private:
    MpiManager();
    ~MpiManager();
private:
...
public:
    ...
    void init() { }
    /// Returns the number of processes
    plint getSize() const { return 1; }
    /// Returns the process ID
    plint getRank() const { return 0; }
    /// Returns process ID of main processor
    plint bossId() const { return 0; }
    /// Tells whether current processor is main processor
