...
std::vector<char>& SendPoolCommunicator::getSendBuffer(plint toProc) {
	if(toProc >= INT_MIN && toProc <= INT_MAX)
	{
		std::map<int,CommunicatorEntry>::iterator entryPtr = subscriptions.find((int)toProc);
	...
	else{ throw std::overflow_error("SendPoolCommunicator::getSendBuffer");}
}

void SendPoolCommunicator::acceptMessage(plint toProc, bool staticMessage)
{
	if(toProc >= INT_MIN && toProc <= INT_MAX)
	{
		std::map<int,CommunicatorEntry>::iterator entryPtr = subscriptions.find((int)toProc);
		...
		if (entry.currentMessage==(int)entry.lengths.size()) {
			startCommunication((int)toProc, staticMessage);
			entry.reset();
		}
	}
	else{ throw std::overflow_error("SendPoolCommunicator::acceptMessage");}
}
...
void SendPoolCommunicator::startCommunication(int toProc, bool staticMessage)
{
    ...
    if (!staticMessage) {
        ...
        global::mpi().iSend(&entry.dynamicDataSizes[0], (int)entry.dynamicDataSizes.size(), toProc,
                            &entry.sizeRequest);
    }
    // Empty messages are neither sent nor received.
    if (!entry.data.empty()) {
        global::profiler().increment("mpiSendChar", (plint)entry.data.size());
        global::mpi().iSend(&entry.data[0], (int)entry.data.size(), toProc, &entry.messageRequest);
    }
}
...
void RecvPoolCommunicator::startBeingReceptive(bool staticMessage)
{
    ...
    for (; iter != subscriptions.end(); ++iter) {
        ...
        if (!entry.data.empty()) {
            ...
            global::mpi().iRecv(&entry.data[0], (int)entry.data.size(),
                                fromProc, &entry.messageRequest);
        }
    }
}

std::vector<char> const& RecvPoolCommunicator::receiveMessage (plint fromProc, bool staticMessage )
{
	if(fromProc >= INT_MIN && fromProc <= INT_MAX)
	{
		std::map<int,CommunicatorEntry>::iterator entryPtr = subscriptions.find((int)fromProc);
		PLB_ASSERT( entryPtr!= subscriptions.end() );
		CommunicatorEntry& entry = entryPtr->second;
		PLB_ASSERT( entry.currentMessage < (int)entry.messages.size() );
		if (entry.currentMessage==0) {
			if (staticMessage) {
				finalizeStatic((int)fromProc);
			}
			else {
				receiveDynamic((int)fromProc);
			}
		}
	...
	else{ throw std::overflow_error("RecvPoolCommunicator::receiveMessage");}
}
...
void RecvPoolCommunicator::receiveDynamic(int fromProc)
{
    ...
    //    are obtained.
    int numMessages = (int)entry.messages.size();
...
void RecvPoolCommunicator::finalizeStatic(int fromProc)
{
    ...
    if (!entry.data.empty()) {
        ...
        for (pluint iMessage=0; iMessage<entry.messages.size(); ++iMessage) {
            plint length = entry.lengths[iMessage];
            entry.messages[iMessage].resize(length);
            PLB_ASSERT(pos+length <= (int)entry.data.size());
