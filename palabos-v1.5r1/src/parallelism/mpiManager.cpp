                                  ...
                                  std::vector<char>& recvVal, MPI_Op op, int root)
{
    ...
    MPI_Reduce(static_cast<void*>(&(tmpSend[0])), recvPtr,(int)sendVal.size(), MPI_INT, op, root, getGlobalCommunicator());
                                 ...
                                 std::vector<int>& recvVal, MPI_Op op, int root)
{
    ...
    MPI_Reduce(static_cast<void*>(&(sendVal[0])), recvPtr,(int)sendVal.size(), MPI_INT, op, root, getGlobalCommunicator());
}
                                  ...
                                  std::vector<long>& recvVal, MPI_Op op, int root)
{
    ...
    MPI_Reduce(static_cast<void*>(&(sendVal[0])), recvPtr,(int)sendVal.size(), MPI_LONG, op, root, getGlobalCommunicator());
}
                                   ...
                                   std::vector<float>& recvVal, MPI_Op op, int root)
{
    ...
    MPI_Reduce(static_cast<void*>(&(sendVal[0])), recvPtr,(int)sendVal.size(), MPI_FLOAT, op, root, getGlobalCommunicator());
}
                                    ...
                                    std::vector<double>& recvVal, MPI_Op op, int root)
{
    ...
    MPI_Reduce(static_cast<void*>(&(sendVal[0])), recvPtr,(int)sendVal.size(), MPI_DOUBLE, op, root, getGlobalCommunicator());
}
                                         ...
                                         std::vector<long double>& recvVal, MPI_Op op, int root)
{
    ...
    MPI_Reduce(static_cast<void*>(&(sendVal[0])), recvPtr,(int)sendVal.size(), MPI_LONG_DOUBLE, op, root, getGlobalCommunicator());
}
                                       ...
                                       std::vector<long long>& recvVal, MPI_Op op, int root)
{
    ...
    MPI_Reduce(static_cast<void*>(&(sendVal[0])), recvPtr,(int)sendVal.size(), MPI_LONG_LONG, op, root, getGlobalCommunicator());
}
#endif
                                         ...
                                         std::vector<__float128 >& recvVal, MPI_Op op, int root)
{
    ...
    MPI_Reduce(static_cast<void*>(&(sendVal[0])), recvPtr,(int)sendVal.size()*sizeof(__float128), MPI_CHAR, op, root, getGlobalCommunicator());
}
#endif
                                              ...
                                              std::vector<Complex<double> >& recvVal, MPI_Op op, int root)
{
    ...
    MPI_Reduce(static_cast<void*>(&(sendVal[0])), recvPtr,2*(int)sendVal.size(), MPI_DOUBLE, op, root, getGlobalCommunicator());
}
                                                   ...
                                                   std::vector<Complex<long double> >& recvVal, MPI_Op op, int root)
{
    ...
    MPI_Reduce(static_cast<void*>(&(sendVal[0])), recvPtr, 2*(int)sendVal.size(), MPI_LONG_DOUBLE, op, root, getGlobalCommunicator());
}
                                                  ...
                                                  std::vector<Complex<__float128> >& recvVal, MPI_Op op, int root)
{
    ...
    MPI_Reduce(static_cast<void*>(&(sendVal[0])), recvPtr, 2*(int)sendVal.size(), MPI_CHAR, op, root, getGlobalCommunicator());
}
#endif
...
void MpiManager::allReduceVect<char>(std::vector<char>& sendRecvVal, MPI_Op op)
{
	...
	if(sendRecvVal.size() >= INT_MIN && sendRecvVal.size() <= INT_MAX){
		std::vector<char> recvVal((int)sendRecvVal.size());
		MPI_Allreduce(static_cast<void*>(&(sendRecvVal[0])), static_cast<void*>(&(recvVal[0])),
			(int)sendRecvVal.size(), MPI_CHAR, op, getGlobalCommunicator() );
		sendRecvVal.swap(recvVal);
	}
	else{ throw std::overflow_error("MpiManager::allReduceVect");}
}
...
void MpiManager::allReduceVect<int>(std::vector<int>& sendRecvVal, MPI_Op op)
{
    ...
    if (sendRecvVal.empty()) return;
	if(sendRecvVal.size() >= INT_MIN && sendRecvVal.size() <= INT_MAX){
		std::vector<int> recvVal((int)sendRecvVal.size());
		MPI_Allreduce( static_cast<void*>(&(sendRecvVal[0])), static_cast<void*>(&(recvVal[0])),
			(int)sendRecvVal.size(), MPI_INT, op, getGlobalCommunicator() );
		sendRecvVal.swap(recvVal);
	}
	else{ throw std::overflow_error("MpiManager::allReduceVect");}
}
...
void MpiManager::allReduceVect<long>(std::vector<long>& sendRecvVal, MPI_Op op)
{
	...
	if (sendRecvVal.empty()) return;
	if(sendRecvVal.size() >= INT_MIN && sendRecvVal.size() <= INT_MAX){
		std::vector<long> recvVal((int)sendRecvVal.size());
		MPI_Allreduce( static_cast<void*>(&(sendRecvVal[0])), static_cast<void*>(&(recvVal[0])),
			(int)sendRecvVal.size(), MPI_LONG, op, getGlobalCommunicator() );
		sendRecvVal.swap(recvVal);
	}
	else{ throw std::overflow_error("MpiManager::allReduceVect");}
}
...
void MpiManager::allReduceVect<float>(std::vector<float>& sendRecvVal, MPI_Op op)
{
	...
	if (sendRecvVal.empty()) return;
	if(sendRecvVal.size() >= INT_MIN && sendRecvVal.size() <= INT_MAX){
		std::vector<float> recvVal((int)sendRecvVal.size());
		MPI_Allreduce( static_cast<void*>(&(sendRecvVal[0])), static_cast<void*>(&(recvVal[0])),
			(int)sendRecvVal.size(), MPI_FLOAT, op, getGlobalCommunicator() );
		sendRecvVal.swap(recvVal);
	}
	else{ throw std::overflow_error("MpiManager::allReduceVect");}
}
...
void MpiManager::allReduceVect<double>(std::vector<double>& sendRecvVal, MPI_Op op)
{
	...
	if (sendRecvVal.empty()) return;
	if(sendRecvVal.size() >= INT_MIN && sendRecvVal.size() <= INT_MAX){
		std::vector<double> recvVal((int)sendRecvVal.size());
		MPI_Allreduce( static_cast<void*>(&(sendRecvVal[0])), static_cast<void*>(&(recvVal[0])),
			(int)sendRecvVal.size(), MPI_DOUBLE, op, getGlobalCommunicator() );
		sendRecvVal.swap(recvVal);
	}
	else{ throw std::overflow_error("MpiManager::allReduceVect");}
}
...
void MpiManager::allReduceVect<long double>(std::vector<long double>& sendRecvVal, MPI_Op op)
{
	...
	if (sendRecvVal.empty()) return;
	if(sendRecvVal.size() >= INT_MIN && sendRecvVal.size() <= INT_MAX){
		std::vector<long double> recvVal((int)sendRecvVal.size());
		MPI_Allreduce( static_cast<void*>(&(sendRecvVal[0])), static_cast<void*>(&(recvVal[0])),
			(int)sendRecvVal.size(), MPI_LONG_DOUBLE, op, getGlobalCommunicator() );
		sendRecvVal.swap(recvVal);
	}
	else{ throw std::overflow_error("MpiManager::allReduceVect");}
}
...
void MpiManager::allReduceVect<Complex<double> >(std::vector<Complex<double> >& sendRecvVal, MPI_Op op)
{
	...
	if (sendRecvVal.empty()) return;
	if(sendRecvVal.size() >= INT_MIN && sendRecvVal.size() <= INT_MAX){
		std::vector<Complex<double> > recvVal((int)sendRecvVal.size());
		MPI_Allreduce( static_cast<void*>(&(sendRecvVal[0])), static_cast<void*>(&(recvVal[0])),
			2*(int)sendRecvVal.size(), MPI_DOUBLE, op, getGlobalCommunicator() );
		sendRecvVal.swap(recvVal);
	}
	else{ throw std::overflow_error("MpiManager::allReduceVect");}
}
...
void MpiManager::allReduceVect<Complex<long double> >(std::vector<Complex<long double> >& sendRecvVal, MPI_Op op)
{
	...
	if (sendRecvVal.empty()) return;
	if(sendRecvVal.size() >= INT_MIN && sendRecvVal.size() <= INT_MAX){
		std::vector<Complex<long double> > recvVal((int)sendRecvVal.size());
		MPI_Allreduce( static_cast<void*>(&(sendRecvVal[0])), static_cast<void*>(&(recvVal[0])),
			2*(int)sendRecvVal.size(), MPI_LONG_DOUBLE, op, getGlobalCommunicator() );
		sendRecvVal.swap(recvVal);
	}
	else{ throw std::overflow_error("MpiManager::allReduceVect");}
}
