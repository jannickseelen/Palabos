...
struct PoolEntry {
    ...
    { }
    plint cumDataLength;
    std::vector<plint> lengths;
};

/// This is a "write-only" storage for the full communication pattern on a processor.
class SendRecvPool {
public:
    typedef std::map<plint, PoolEntry> SubsT;
public:
    void subscribeMessage(plint proc, plint numData) {
...
struct CommunicatorEntry {
    ...
    ///   blocking communication pattern and avoids unnecessery de- and re-
    ///   allocations.
    std::vector<plint> dynamicDataSizes;
    int currentMessage;
...
public:
    ...
    std::vector<char>& getSendBuffer(plint toProc);
    void acceptMessage(plint toProc, bool staticMessage);
...
public:
    ...
    std::vector<char> const& receiveMessage(plint fromProc, bool staticMessage);
private:
