        ...
        plint sizeOfCell )
{
    ...
    for (pluint iOverlap=0; iOverlap<overlaps.size(); ++iOverlap) {
        ...
        info.fromBlockId = (int)overlap.getOriginalId();
        info.toBlockId   = (int)overlap.getOverlapId();
        ...
        MultiBlock3D& destinationMultiBlock, modif::ModifT whichData ) const
{
    ...
    for (unsigned iSend=0; iSend<communication.sendPackage.size(); ++iSend) {
        ...
        fromBlock.getDataTransfer().send(info.fromDomain, communication.sendComm.getSendBuffer(info.toProcessId),whichData);
        ...
        MultiBlock3D& destinationMultiBlock, modif::ModifT whichData ) const
{
    ...
    for (unsigned i=0; i<communication.sendPackage.size(); ++i) {
        ...
        global::mpi().iSend(&dataSizes[i], 1, (int)info.toProcessId, &request1[i]);
        if(dataSizes[i]>0) {
            global::mpi().iSend(&data[i][0], (int)dataSizes[i], (int)info.toProcessId, &request2[i]);
        }
    }
    ...
    for (unsigned i=0; i<communication.recvPackage.size(); ++i) {
        ...
        int dataSize;
        global::mpi().receive(&dataSize, 1, (int)info.fromProcessId);
        if (dataSize>0) {
