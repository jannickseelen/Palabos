...
plint Dynamics<T,Descriptor>::getId() const {
    return 0;
}
...
template<typename T, template<typename U> class Descriptor>
plint NoDynamics<T,Descriptor>::id =
...
template<typename T, template<typename U> class Descriptor>
plint NoDynamics<T,Descriptor>::getId() const {
    return id;
}
...
void serialize(Dynamics<T,Descriptor> const& dynamics, std::vector<char>& data) {
    HierarchicSerializer serializer(data, (int)dynamics.getId());
