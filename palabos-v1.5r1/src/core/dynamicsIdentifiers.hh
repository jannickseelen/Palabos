...
template<typename T, template<typename U> class Descriptor>int DynamicsRegistration<T,Descriptor>::announce(std::string nameOfDynamics,
DynamicsGenerator<T,Descriptor>* generator){
	...
	int nextId = 0;
	if( sizeof(dynamicsByNumber.size() < sizeof(int))){
		nextId = (int)dynamicsByNumber.size();
	}
	else{
		throw std::overflow_error("DynamicsRegistration<T,Descriptor>::announce");
	}
